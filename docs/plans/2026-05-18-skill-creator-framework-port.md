...
- **Workspace volume bloat.** Skill authoring writes a lot of files per iteration. Honor the per-tenant workspace quota enforced by `apps/api/app/api/v1/workspace.py` (`_TENANT_WORKSPACE_BUDGET`, default **1 GiB**, env-tunable via `TENANT_WORKSPACE_BUDGET_BYTES`). Phase 2's `dispatch_iteration` does a precheck through `_tenant_workspace_bytes` and raises `TenantWorkspaceQuotaExceeded` → HTTP 413 before queueing any rows. The code-worker `tenant_home_quota.py` separately bounds the code-worker's HOME (a 2 GiB soft cap on CLI caches, unrelated to eval artifacts). Or carve a separate `skill_evals/` mount under the workspaces volume with its own quota.
