...
> **WONTFIX (2026-05-25).** Attempted across PRs #712, #720, #721, #723, #724. The runner subprocess sees the login keychain as locked (Mach security session isolation, not auto-lock). Cannot unlock without storing the login password — chicken-and-egg. Disk-theft + backup-hygiene was the realistic gain; not worth the complexity. Refocused on F7b/F7c which close real takeover-chain risk. Full decision: [`docs/plans/2026-05-25-f2-keychain-wontfix-decision.md`](../../plans/2026-05-25-f2-keychain-wontfix-decision.md). The R1 claim at line 41 ("Keychain on the runner is feasible without interactive unlock") was wrong — attribute reads work cross-session, but `-w` value reads need a per-session unlock event.
