...
def execute(inputs: dict) -> dict:
    ...
    if not query: return {"error": "Missing required input: query"}
    lang = inputs.get("language", "").strip()
    num = min(int(inputs.get("num_results", 5)), 10)
    search_q = f"{lang} {query}" if lang else query
    # Search with site preference for tech sources
    results = _search(f"{search_q} site:stackoverflow.com OR site:github.com", num)
    if len(results) < 3:
        general = _search(f"{search_q} documentation", num - len(results))
        seen = {r["url"] for r in results}
        results.extend(r for r in general if r["url"] not in seen)
    results = results[:num]
    engine = results[0]["source"] if results else "none"
    return {"query": query, "language": lang or "any",
