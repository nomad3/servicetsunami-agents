...
def execute(inputs: dict) -> dict:
    ...
    img_type = inputs.get("image_type", "").strip()
    extra = {"searchType": "image"}
    if img_type:
        extra["imgType"] = img_type
    results = _google_search(query, num, **extra)
    if not results:
        results = _search(f"{query} images", num)
    engine = results[0]["source"] if results else "none"
    return {"query": query, "image_type": img_type or "any",
