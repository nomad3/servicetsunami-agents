...
def execute(inputs: dict) -> dict:
    name = inputs.get("person_name", "").strip()
    if not name: return {"error": "Missing required input: person_name"}
    ctx = inputs.get("context", "").strip()
    num = min(int(inputs.get("num_results", 5)), 10)
    queries = [
        f"{name} {ctx} LinkedIn" if ctx else f"{name} LinkedIn",
        f"{name} {ctx}" if ctx else name,
    ]
    all_results = []
    seen_urls = set()
    for q in queries:
        for r in _search(q, num):
    ...
    return {"person": name, "context": ctx or "none",
