...
def execute(inputs: dict) -> dict:
    biz = inputs.get("business_type", "").strip()
    if not biz: return {"error": "Missing required input: business_type"}
    loc = inputs.get("location", "").strip()
    num = min(int(inputs.get("num_results", 5)), 10)
    if loc:
        query = f"best {biz} in {loc} reviews"
    else:
        query = f"best {biz} near me reviews"
    results = _search(query, num)
    engine = results[0]["source"] if results else "none"
    return {"business_type": biz, "location": loc or "not specified",
