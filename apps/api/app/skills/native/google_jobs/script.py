...
def execute(inputs: dict) -> dict:
    ...
    if not role: return {"error": "Missing required input: role"}
    loc = inputs.get("location", "remote").strip()
    level = inputs.get("experience_level", "").strip()
    num = min(int(inputs.get("num_results", 7)), 10)
    parts = [role]
    ...
    return {"role": role, "location": loc, "experience_level": level or "any",
