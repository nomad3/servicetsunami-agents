...
def execute(inputs: dict) -> dict:
    ...
    if not task: return {"error": "Missing required input: task"}
    num = min(int(inputs.get("num_results", 5)), 10)
    level = inputs.get("skill_level", "").strip()
    query = f"how to {task} tutorial guide"
    if level:
        query = f"how to {task} {level} tutorial guide"
    results = _search(query, num)
    engine = results[0]["source"] if results else "none"
    return {"task": task, "skill_level": level or "any",
