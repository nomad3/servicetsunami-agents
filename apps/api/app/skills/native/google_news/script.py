...
def execute(inputs: dict) -> dict:
    ...
    tf = inputs.get("timeframe", "week").lower()
    date_restrict = TIMEFRAME_MAP.get(tf, "w1")
    query = f"{topic} news"
    results = _search(query, num, sort="date", dateRestrict=date_restrict)
    if not results:
        results = _search(query, num)
    engine = results[0]["source"] if results else "none"
    return {"topic": topic, "timeframe": tf, "results": results,
