...
def execute(inputs: dict) -> dict:
    items = inputs.get("items", "").strip()
    if not items: return {"error": "Missing required input: items"}
    criteria = inputs.get("criteria", "").strip()
    num = min(int(inputs.get("num_results", 5)), 10)
    query = f"{items} comparison review"
    if criteria:
        query = f"{items} comparison {criteria}"
    results = _search(query, num)
    if len(results) < 3:
        alt = _search(f"{items} vs", num - len(results))
        seen = {r["url"] for r in results}
        results.extend(r for r in alt if r["url"] not in seen)
    results = results[:num]
    engine = results[0]["source"] if results else "none"
    return {"items": items, "criteria": criteria or "general",
            "results": results, "result_count": len(results), "engine": engine}
